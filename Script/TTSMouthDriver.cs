...
public class TTSMouthDriver : MonoBehaviour
{
    ...
    [Header("Sync")]
    [Tooltip("音声出力の遅延補正（+で口を遅らせる, 単位 ms）。負にすると口が先行します。")]
    public float lipDelayMs = -200f;

    [Tooltip("true: minPerVowel未満のチャンクは次の有声と結合")]
    public bool mergeTooShort = true;

    [Header("Catch-up (遅延溜まり対策)")]
    [Tooltip("口パクの歩幅・待ち時間に掛ける全体スケール。1.0 = 既定速度（<1.0 で速送り）")]
    [Range(0.5f, 1.5f)] public float timeScale = 1.0f;

    [Tooltip("キューが MaxQueue に近づいたら一時的に何倍速にするか（1.0〜0.5推奨）")]
    [Range(0.5f, 1.0f)] public float adaptiveMinScale = 0.7f;

    [Tooltip("受信キューの上限。超えた分は古いものから間引き、遅延の雪だるまを防ぎます。")]
    [Min(2)] public int MaxQueue = 10;
    ...
    IEnumerator ConsumeQueue()
    {
        ...
        float initDelay = lipDelayMs * 0.001f;
        if (initDelay > 0f) yield return new WaitForSecondsRealtime(initDelay);
        ...
        while (true)
        {
            ...
            // 溜まり具合に応じて一時的に速送り（歩幅と待ち時間を圧縮）
            float adaptive =
                Mathf.Lerp(adaptiveMinScale, 1.0f, Mathf.InverseLerp(MaxQueue, 0, _queue.Count));
            float scale = Mathf.Clamp(timeScale * adaptive, 0.5f, 2.0f);
            ...
            {
                SetNeutralTargets();
                yield return new WaitForSecondsRealtime(Mathf.Max(neutralHold, dur) * scale);
                continue;
            }
            ...
            if (mergeTooShort)
            {
                ...
                while (_queue.Count > 0 && dur < minPerVowel * e.vowels.Count)
                {
                    ...
                    dur += next.dur;
                    e.vowels.AddRange(next.vowels); // vowels を後ろに連結
                    _queue.Dequeue();
                }
            ...
            // 最後の残り分 + 余韻（こちらもスケール）
            yield return new WaitForSecondsRealtime(per * Mathf.Clamp01(overlap) + 0.01f * scale);
            SetNeutralTargets();
        }
    }
